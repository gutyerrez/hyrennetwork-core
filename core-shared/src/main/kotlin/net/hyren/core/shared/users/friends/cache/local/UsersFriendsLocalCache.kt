...
class UsersFriendsLocalCache : LocalCache {
    ...
    private val FRIENDS_CACHE = Caffeine.newBuilder()
        ...
        .build<EntityID<UUID>, List<FriendUser>> {
            CoreProvider.Repositories.PostgreSQL.USERS_FRIENDS_REPOSITORY.provide().fetchByUserId(
    ...
    private val FRIEND_REQUESTS_CACHE = Caffeine.newBuilder()
        .expireAfterWrite(15, TimeUnit.SECONDS)
        .build<EntityID<UUID>, List<FriendUser>> {
            CoreProvider.Repositories.PostgreSQL.USERS_FRIENDS_REPOSITORY.provide().fetchFriendRequestsByUserId(
